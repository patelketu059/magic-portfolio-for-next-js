...
- Implemented the full probabilistic encoder–decoder pipeline:
  ...
  - **VAE loss** combining **reconstruction loss (L1/L2)** and **KL-divergence regularization**:
    ...
    ```text
    L_total = L_recon + β * D_KL(qφ(z|x) || p(z))
    ```
...
| **Datasets** | MNIST and Fashion-MNIST (28×28 grayscale, 10 classes) |
| **Architectures** | VAE (Encoder–Decoder MLP), GAN (Generator + Discriminator), DDPM (UNet) |
...
| **Noise Scheduling (Diffusion)** | Linear variance schedule βₜ ∈ [0.0001, 0.02] |
...
| Model | Dataset | FID ↓ | Observations |
|--------|----------|-------|---------------|
